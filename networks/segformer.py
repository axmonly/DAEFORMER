...
# gpt说会有错误。
# LocalEnhance_EfficientSelfAtten 类中的参数错误
# LocalEnhance_EfficientSelfAtten 类的 forward 方法在调用 self.local_pos 时，没有传入必要的 H 和 W 参数，这会导致 TypeError。
...
class MixFFN_skip(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.act = nn.GELU()
        self.fc2 = nn.Linear(c2, c1)
        # 稳定训练过程（防止梯度爆炸/消失），Transformer 模型中的每一层前都加 LayerNorm
...
class MixD_FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 这里有错误，应该是out = self.fc2(ax)
        out = self.fc2(ax)
        return out


class OverlapPatchEmbeddings(nn.Module):
    # 定义的是卷积核的输入和输出通道  in_ch=3, dim=768
    def __init__(self, img_size=224, patch_size=7, stride=4, padding=1, in_ch=3, dim=768):
        super().__init__()
        self.num_patches = (img_size // patch_size) ** 2
        # self.proj = nn.Conv2d(3, 768, kernel_size=7, stride=4, padding=1)
        self.proj = nn.Conv2d(in_ch, dim, patch_size, stride, padding) # 
        self.norm = nn.LayerNorm(dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 假设x = torch.randn(2, 3, 224, 224)  # [B, C, H, W]
        px = self.proj(x)           
        _, _, H, W = px.shape     #执行后  px.shape = [2, 768, 55, 55]
        fx = px.flatten(2).transpose(1, 2) # [2, 768, 3025]
        #  对最后一个维度（特征维）做归一化：
        nfx = self.norm(fx)  # [2, 768, 3025]
        return nfx, H, W # nfx: [2, 3025, 768] , H = 55, W = 55
